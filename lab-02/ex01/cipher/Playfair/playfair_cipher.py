class PlayFairCipher:
    ...
    def create_playfair_matrix(self, key):
        key = key.replace("J","I")
        key = key.upper()
        key_set = set(key)  # Fix: Create a set from the key
    ...
    def playfair_encrypt(self, plain_text, matrix):
        ...
        for i in range(0, len(plain_text), 2):
            ...
            row2, col2 = self.find_letter_coords(matrix, pair[1])  # Fix: Use pair[1] instead of [1]
            if row1 == row2:
    ...
    def playfair_decrypt(self, cipher_text, matrix):
        ...
        if decrypted_text[-1] != decrypted_text[-2]:
